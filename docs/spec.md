...
| 運営主体 | 一般社団法人コード・フォー・ジャパン（フッタ・ポリシーページに明記する） |
...
| 8 | 無検証・完全公開でありながら、誤分類やレポートの誤りを外部から指摘する経路が存在しない | **GitHub Issues（`codeforjapan/kumamoto-earthquake-dashboard`）を訂正・削除依頼の窓口とし、フッタとポリシーページに常時掲示する**（v1.1 で追加。それ以前は「緩和策なし・受容リスク」としていた）。事後対応であり、公開前検証を行うことを意味しない点をポリシーページに明記する。窓口が機能する前提としてリポジトリを公開し Issues を有効にしておく必要がある |
