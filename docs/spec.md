...
1. **サイト名と対外文言を「誤情報」断定から「コミュニティノート」の提示に変更**。収集対象には「評価不足」「有用でない」と評価されたノートも含まれるため、サイト側が元投稿を虚偽と判定しているという読み方を成立させない。分類器・レポート生成の内部プロンプトが用いる「誤情報・撹乱情報」という判断基準（§4）は精度に直結するため変更しない（※その後 v3 で判断基準を「今回の熊本地震に関係するか」へ変更した。詳細は §4。）
...
| 関連性フィルタ | LLMがスコア0-100を付与。**閾値60未満を除外**（ソフト除外）。評価軸は「今回の熊本地震に関係するか」であり、除外は"明確に無関係"な話題のみ（訂正の形でない地震関連情報も対象）（§4 Stage 1） |
...
**スコアが測るのは「今回の熊本地震に関係するか」であり、「誤情報を"訂正"しているか」ではない。**

今回の熊本地震に関係する内容であれば、訂正の形をとっていなくても（被害報告・安否・義援金の案内、時系列や事実関係の指摘、地震絡みの政治的論評、別災害の映像が今回のものと誤って紐付けられた件への言及など）すべて対象に含める。除外するのは、地震と**明確に無関係**で、たまたま「熊本」「地震」の語を含むだけの話題（医療デマ、アニメのAI生成判定、地震と無関係な政治・統計の話題など）のみである。

> 「訂正」の形式を要求する狭い基準では、被害報告や安否確認のような訂正の形でない地震関連情報まで取りこぼす。キーワード検索（熊本 / 地震 / 津波）だけでは無関係ノートが大量に混入するためこのステージは必須だが、判定軸を厳密にしすぎてもモニタの目的（地震周辺の情報を幅広く記録すること）を損なうため、迷う場合は残す方向に倒す。
