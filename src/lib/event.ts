 ...
 *
 * keyMoment* について: 「本震」（発生時刻・マグニチュード）は地震固有の概念であり、
 * 選挙等の別テーマでは「投開票日」のように意味も表示フォーマットも変わる
 * （マグニチュードに相当する値が無い等）。そのため呼び出し側（StatCards・チャート）は
 * 「ラベル文字列」と「表示用に整形済みの値文字列」だけを受け取り、フォーマットの組み立て
 * （「{マグニチュード} / {時刻}」のような地震固有の形）はすべてここに閉じ込めてある。
 */

const OCCURRED_AT = Date.parse("2026-07-28T16:27:00+09:00");
const MAGNITUDE = "M7.1";
...
export const EVENT = {
  ...
  // ── 基準時刻（タイムライン上のマーカー・統計カード等で使う） ────────
  /** epoch ms。タイムライン上に縦の基準線を引く「注目すべき瞬間」。 */
  occurredAt: OCCURRED_AT,

  // ── 統計カード・チャート注釈の「本震」枠 ──────────────────────
  /** カードの見出し・チャート注釈の接頭辞として共通で使う短いラベル。 */
  keyMomentLabel: "本震",
  /**
   * StatCardsの「本震」カードに表示する、整形済みの値。
   * 地震固有の「マグニチュード / 時刻」という形式はここで組み立て済みにし、
   * StatCards.tsx 側にはこのフォーマットの知識を持たせない。
   */
  keyMomentValue: `${MAGNITUDE} / ${hhmm(OCCURRED_AT)}`,
