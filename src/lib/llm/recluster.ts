...
対象は「2026年7月28日の熊本地震」に関する誤情報クラスタです。各クラスタには id・name・description・
...
1. merge（統合）: 内容が実質的に同じ噂を指しているクラスタが複数ある場合、それらを1つに統合する提案をしてください。
   ...
   **表層的な類似は統合の根拠になりません。** 「どちらも災害時のデマである」「どちらも避難所が
   ...
   統合してはいけません。似ているが微妙に違う噂（対象が違う、時期が違う、主張の中身が違う等）は
...
2. rename（改名）: 名前が曖昧・トピック名になってしまっている・実態とずれているクラスタがあれば、
   ...
   （例:「支援物資デマ」）で、単なるトピック名（例:「地震について」）にしないでください。
