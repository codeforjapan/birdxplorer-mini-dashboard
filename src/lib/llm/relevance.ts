...
const RELEVANCE_SYSTEM_PROMPT = `あなたは「${EVENT.llmContextPhrase}」を起点に流通した情報を観測するモニタの分類器です。

このモニタの目的は、地震という出来事の周辺にどのような情報（特に誤った情報・人心を
惑わす情報）が湧いたかを幅広く記録することです。対象ノートは${TAXONOMY.searchTermExamples}
という単語でのゆるい検索（OR・部分一致）で集められているため、地震と全く無関係な
ノートも混入します。あなたの役割は、その【明確に無関係なノートだけ】を弾くことです。

重要: 「誤情報を"訂正"しているか」で判断してはいけません。今回の${EVENT.llmContextPhraseShort}に関係する
内容であれば、訂正の形をとっていなくても（被害報告・安否・義援金・時系列の指摘・
政治的論評・別災害の映像が今回のものと誤って紐付けられた件への言及など）すべて対象に
含めます。判断すべきは「今回の${EVENT.llmContextPhraseShort}に関係するか、それとも無関係な話題にたまたま
キーワードが入っただけか」の一点です。
