import { isTooSoon, MIN_SYNC_INTERVAL_MS, runCronJob } from "@/lib/cron";
...
export async function GET(req: Request): Promise<Response> {
  ...
  return runCronJob("searchlight-sync", req, async (): Promise<Record<string, number>> => {
    ...
    if (!force && isTooSoon(await getRun("searchlight-sync"), Date.now(), MIN_SYNC_INTERVAL_MS)) {
      return { skipped: 1 };
    }
    ...
    const rows = await getInsights(session, now);
    ...
    for (const platform of CROSS_PLATFORMS) {
      const crossRows = await getCrossInsightsFor(session, platform, now);
      await upsertCrossPosts(crossRows);
      crossUpserted += crossRows.length;
    }

    // requests を記録しておくと、job_runs だけで呼び出し回数の増加に気づける。
    return { upserted: rows.length, crossUpserted, requests: session.requestCount };
  });
}
